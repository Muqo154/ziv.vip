...
# ========== FIX: Tambahkan fungsi cleanup binary ==========
cleanup_binary() {
...
# ========== FIX: Perbaiki download_binary ==========
download_binary() {
    log "${YELLOW}Detecting architecture...${NC}"
    
    # Cleanup existing binary first
    cleanup_binary
    ...
    # Prioritaskan download dari repo Pondok-Vpn dulu
    LOCAL_REPO_URL="https://raw.githubusercontent.com/Pondok-Vpn/udp-ziv/main/$BINARY_NAME"
    
    log "${CYAN}Trying from Pondok-Vpn repo...${NC}"
    if wget --timeout=30 --tries=2 -q "$LOCAL_REPO_URL" -O /usr/local/bin/zivpn; then
        if [ -f /usr/local/bin/zivpn ]; then
            FILE_SIZE=$(stat -c%s /usr/local/bin/zivpn 2>/dev/null || echo 0)
            if [ $FILE_SIZE -gt 1000000 ]; then
                chmod +x /usr/local/bin/zivpn
                echo -e "${GREEN}✓ Binary downloaded from Pondok-Vpn ($((FILE_SIZE/1024/1024))MB)${NC}"
                return 0
            fi
        fi
    fi
...
echo -e "${BLUE}  ═══════════════════════════════════════════════════════${NC}"
    sleep 3
      # FIX: Cek permission sebelum menjalankan menu
      if [ -f "$MENU_SCRIPT" ]; then
        if [ -x "$MENU_SCRIPT" ]; then
            "$MENU_SCRIPT"
        else
