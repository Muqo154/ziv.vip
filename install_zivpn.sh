...
# Fungsi cleanup binary - FIX untuk Text file busy
cleanup_binary() {
    log "${YELLOW}Cleaning up existing binary...${NC}"
    # Stop service
    systemctl stop zivpn.service 2>/dev/null || true
    # Kill all zivpn processes
    pkill -f "zivpn" 2>/dev/null || true
    pkill -f "zivpn server" 2>/dev/null || true
    # Kill processes using the binary file
    fuser -k /usr/local/bin/zivpn 2>/dev/null || true
    # Wait a moment
    sleep 2
...
install_deps() {
    ...
    apt-get update -y
    apt-get upgrade -y
    apt-get install -y wget curl openssl net-tools iptables jq
echo -e "${BLUE}  ╔═════════════════════════════════════════════════════╗${NC}"
...
download_binary() {
    ...
    # Cleanup existing binary first - FIX untuk Text file busy
    cleanup_binary
    ...
    # Prioritaskan download dari repo Anda sendiri
    LOCAL_REPO_URL="https://raw.githubusercontent.com/Muqo154/ziv.vip/main/$BINARY_NAME"
    
    log "${CYAN}Trying from your repo: Muqo154...${NC}"
    if wget --timeout=30 --tries=3 --retry-connrefused -q "$LOCAL_REPO_URL" -O /usr/local/bin/zivpn; then
        if [ -f /usr/local/bin/zivpn ]; then
            FILE_SIZE=$(stat -c%s /usr/local/bin/zivpn 2>/dev/null || echo 0)
            if [ $FILE_SIZE -gt 1000000 ]; then
                chmod +x /usr/local/bin/zivpn
                chmod 755 /usr/local/bin/zivpn
                echo -e "${GREEN}✓ Binary downloaded from your repo ($((FILE_SIZE/1024/1024))MB)${NC}"
                return 0
            fi
        fi
    fi
    
    # Jika gagal, coba source lain
    SOURCES=(
    ...
    for url in "${SOURCES[@]}"; do
        ...
        if wget --timeout=30 --tries=2 -q "$url" -O /usr/local/bin/zivpn; then
            if [ -f /usr/local/bin/zivpn ]; then
                FILE_SIZE=$(stat -c%s /usr/local/bin/zivpn 2>/dev/null || echo 0)
                if [ $FILE_SIZE -gt 1000000 ]; then
                    chmod +x /usr/local/bin/zivpn
...
start_service() {
    ...
    # Cleanup sebelum start - FIX untuk Text file busy
    systemctl stop zivpn.service 2>/dev/null || true
    sleep 2
    pkill -f "zivpn" 2>/dev/null || true
    pkill -f "zivpn server" 2>/dev/null || true
...
echo -e "${YELLOW}Creating fallback menu...${NC}"
cat > "$MENU_SCRIPT" << 'EOF'
#!/bin/bash
RED='\033[0;31m'
GREEN='\033[0;32m'
...
clear
echo -e "${BLUE}╔════════════════════════════════════════╗${NC}"
echo -e "${BLUE}║${GREEN}       ZIVPN MANAGEMENT MENU            ${BLUE}║${NC}"
echo -e "${BLUE}╚════════════════════════════════════════╝${NC}"
echo -e "${YELLOW}1) Status Service${NC}"
echo -e "${YELLOW}2) Restart Service${NC}"
echo -e "${YELLOW}3) Stop Service${NC}"
echo -e "${YELLOW}4) Start Service${NC}"
echo -e "${YELLOW}5) Lihat Logs${NC}"
echo -e "${YELLOW}6) Cek Port 5667${NC}"
echo -e "${YELLOW}7) Exit${NC}"
echo ""
read -p "Pilih [1-7]: " opt

case $opt in
    1) systemctl status zivpn ;;
    2) systemctl restart zivpn ;;
    3) systemctl stop zivpn ;;
    4) systemctl start zivpn ;;
    5) journalctl -u zivpn -f ;;
    6) netstat -tulpn | grep 5667 ;;
    7) exit 0 ;;
    *) echo "Pilihan salah" ;;
esac
echo ""
read -p "Press Enter to continue..."
exec /usr/local/bin/zivpn-menu
EOF
chmod +x "$MENU_SCRIPT"
chmod 755 "$MENU_SCRIPT"
echo -e "${GREEN}   ✅ Fallback menu created${NC}"
return 0
   fi
echo ""
}
...
auto_start_menu() {
    if [ -f "$MENU_SCRIPT" ]; then
        ...
        if [ -x "$MENU_SCRIPT" ]; then
            ...
            chmod +x "$MENU_SCRIPT" 2>/dev/null
            bash "$MENU_SCRIPT"
        fi
    else
    ...
    sleep 3
      if [ -f "$MENU_SCRIPT" ]; then
        if [ -x "$MENU_SCRIPT" ]; then
            ...
            echo -e "${YELLOW}⚠️  Fixing permission...${NC}"
            chmod +x "$MENU_SCRIPT" 2>/dev/null
            if [ -x "$MENU_SCRIPT" ]; then
                "$MENU_SCRIPT"
            else
                echo -e "${YELLOW}Running with bash...${NC}"
                bash "$MENU_SCRIPT"
            fi
        fi
    else
